    ...
    def dodajKontoDoTabeli(self):
        ...
        if mozna == 1:
            if self.ktoSK1 == "customer":
                ...
                kursorSK.execute('select supplierid from servicesupplier')
                indeksy = []
                for i in kursorSK:
                    indeksy.append(i[0])
                ...
                while i in indeksy:
                    i+=1
                kursorSK.execute('insert into servicesupplier (supplierId, companyName, contactName, phone, email, homePage, nick, pswd) values (%s,%s,%s,%s,%s,%s,%s,%s)', (str(i), self.polaSK1[0].text(), self.polaSK1[1].text(), self.polaSK1[2].text(), self.polaSK1[3].text(), self.polaSK1[4].text(),self.nickSK1, self.pswdSK1))
                mydbSK.commit()
            self.logowanie()
...
#okno do ogladania rekordów - gotowe
    def zobaczRekordy(self):
        self.mydbZR = mysql.connector.connect(
            host="localhost",
            user='root',
            passwd='123123123',
            database="test"
            )
        self.kursorZR1 = self.mydbZR.cursor()
        ...
        self.lZR1.show()
        self.lZR2.show()
        self.kursorZR1.execute('Show tables')
        self.cbZR1.clear()
        for i in self.kursorZR1:
            self.cbZR1.addItem(i[0])
        self.kursorZR1.close()
        self.cbZR1.adjustSize()
        self.cbZR1.move(self.szerokosc/2-self.cbZR1.size().width()/2,self.wysokosc*0.08)
        self.cbZR1.show()
        self.kursorZR2 = self.mydbZR.cursor()

        self.kursorZR2.execute('show columns from category')
        self.cbZR2.addItem('*')
        self.columnsZR1 = 1
        for i in self.kursorZR2:
            self.cbZR2.addItem(i[0])
            self.columnsZR1+=1
        self.kursorZR2.close()
    ...
    def updateState(self):
        ...
        self.cbZR2.clear()
        self.cbZR2.addItem('*')
        self.kursorZR3 = self.mydbZR.cursor()

        self.kursorZR3.execute('show columns from '+ self.columnsZR2)
        for i in self.kursorZR3:
            self.cbZR2.addItem(i[0])
        self.kursorZR3.close()
        self.cbZR2.adjustSize()
        self.cbZR2.move(self.szerokosc/2-self.cbZR2.size().width()/2,self.wysokosc*0.15)

    def showData(self):
        self.kursorZR = self.mydbZR.cursor()

        if self.cbZR2.currentText() == "*":
            self.kursorZR.execute('show columns from '+self.cbZR1.currentText())
            kolumny = 0
            for i in self.kursorZR:
                kolumny +=1
            self.kursorZR.close()
            self.tZR1.clear()
            ...
            self.kursorZR.execute('Select '+ self.cbZR2.currentText()+ ' from '+ self.cbZR1.currentText())
            wiersze = 0
            for i in self.kursorZR:
                wiersze+=1
            self.kursorZR.close()
...
#okno do dodawania rekordow
    ...
    def pokazPolaDoWypelnienia(self):
        kursor1.execute('show columns from '+self.cbDR1.currentText())
        for i in self.polaSK1:
            i.hide()
        for i in self.nazwySK2:
            i.hide()
    ...
    def dodajRekordyDoTabel(self):
        if self.cbDR1.currentText() != 'orderdetails':
            ...
            kursor1.execute('select '+idd+'id from '+self.cbDR1.currentText())
            indeksy = []
            for i in kursor1:
                indeksy.append(i[0])
            ...
            kursor1.execute('insert into '+ self.cbDR1.currentText() + ' (' +idd+'id, ' +kolumny +') values ('+str(i)+', '+wartosci+')')
            ...
            if self.polaSK1[6].text() == '1':
                ...
                kolumny = kolumny[0:-2]
                wartosci = wartosci[0:-2]
                kursor1.execute('insert into '+ self.cbDR1.currentText() + ' (' +kolumny +') values ('+wartosci+')')
